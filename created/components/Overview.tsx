...
export default function Overview({ projectId }: OverviewProps) {
  ...
  async function handleDeleteAll() {
    if (!confirm("Delete ALL surveys and ALL responses for this project? This cannot be undone.")) return;
    setDeleting(true);
    try {
      for (const survey of surveys) await deleteResponses(survey.id);
      for (const survey of surveys) await deleteSurveys(projectId, survey.id);
      setSurveys([]);
      setResponsesCounts({});
      setTextResponses({});
      ...
      console.error(err);
      alert("Failed to delete all surveys. See console.");
  ...
  async function handleDelete(surveyId: string) {
    ...
    try {
      ...
      setResponsesCounts(prev => { const n = { ...prev }; delete n[surveyId]; return n; });
      setTextResponses(prev => { const n = { ...prev }; delete n[surveyId]; return n; });
      setStatsDialogOpen(false);
      setSelectedSurvey(null);
    } catch (err) {
      console.error(err);
      alert("Failed to delete survey. See console.");
    } finally { setDeleting(false); }
  }
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {surveys.length > 0 && (
            <Button variant="ghost" size="sm" className="flex items-center gap-2 text-red-600 hover:bg-red-50" onClick={handleDeleteAll}>
              <Trash size={16} /> Delete All
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => setOrderAsc(!orderAsc)}>
            Sort by Date {orderAsc ? "↑" : "↓"}
          </Button>
          <Button variant="ghost" size="sm" onClick={loadSurveys}>Refresh All</Button>
        </div>
      </div>
      ...
      {surveys.length > 0 && (
        <div className="flex flex-col sm:flex-row gap-2 text-sm">
          ...
          <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
            ...
            <span className="text-green-700">{surveys.reduce((total, survey) => {
              const surveyResponses = survey.type === "text" ? (textResponses[survey.id]?.length || 0) :
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-7xl">
        ...
        {surveys.map((survey) => {
          ...
          const totalResponses = survey.type === "text" ? (textResponses[survey.id]?.length || 0) :
            Object.values(responsesCounts[survey.id] || {}).reduce((s, n) => s + n, 0);
          return (
            <div key={survey.id} className="relative">
              <Card className="shadow-sm">
                {isLoading && <div className="absolute inset-0 bg-white/70 flex items-center justify-center z-10 rounded"><Loader2 className="animate-spin" /></div>}
                <CardHeader className="pb-2">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                    <div className="flex-1">
                      <CardTitle className="text-lg">{survey.question}</CardTitle>
                      <p className="text-xs text-gray-500 mt-1">{survey.type} • {new Date(survey.created_at).toLocaleDateString()}</p>
                    ...
                    <div className="flex items-center gap-2 sm:flex-col sm:gap-1">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-32 p-1">
                          <DropdownMenuItem className="flex items-center gap-2 text-red-600 hover:bg-red-50" onClick={(e) => { e.stopPropagation(); handleDelete(survey.id); }}>
                ...
                <CardContent className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <Button variant="outline" size="sm" className="flex-1" onClick={() => openStatsDialog(survey)}>View Stats</Button>
                    <Button variant="outline" size="sm" className="flex-1" onClick={() => refreshSurveyResponses(survey.id)} disabled={isLoading}>
                      {isLoading && <Loader2 className="animate-spin mr-2" />} Refresh
                    </Button>
        ...
        <DialogContent className="max-w-5xl max-h-[80vh] overflow-y-auto">
            ...
            {selectedSurvey && (
              <div className="flex flex-col gap-2">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Type: {selectedSurvey.type} • Created: {new Date(selectedSurvey.created_at).toLocaleString()}</p>
                </div>

                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm">Color:</span>
                    <div className="h-6 w-6 rounded-md border border-gray-300" style={{ backgroundColor: selectedSurvey.color }} />
                  </div>
                  <Button variant="destructive" size="sm" onClick={() => handleDelete(selectedSurvey.id)} disabled={deleting} className="flex items-center gap-1">
                    {deleting ? <Loader2 className="animate-spin" size={16} /> : <Trash2 size={16} />} <span>Delete Survey</span>
                  </Button>
                </div>
              </div>
            )}
          ...
          <Tabs defaultValue="stats" className="w-full">
                    ...
                    {selectedSurvey.type === "text" ? (
                      <div className="space-y-2">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                          <span className="font-semibold">Free Text Responses ({(textResponses[selectedSurvey.id] || []).length})</span>
                          <div className="flex gap-2 flex-wrap">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => refreshSurveyResponses(selectedSurvey.id)}>Refresh</Button>
                          </div>
                        </div>

                        <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                          {getOrderedTextResponses(selectedSurvey.id).map((t, idx) => (
                            <div key={idx} className="bg-gray-50 border border-gray-200 rounded p-3 text-sm break-words">{t}</div>
                          ))}
                        ...
                        {Object.entries(responsesCounts[selectedSurvey.id] || {})
                          ...
                          .map(([answer, count]) => (
                            <li key={answer} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                              <span className="font-medium break-words">{answer}</span>
            ...
            <TabsContent value="embeds">
              {selectedSurvey ? (
                <Card className="shadow-sm space-y-3 p-3 overflow-x-auto">
                  {["survey_link", "survey_iframe", "survey_script", "survey_widget", "survey_react_component"].map(key => (
                    <div key={key} className="mb-2">
                      ...
                      <pre className="bg-gray-100 p-1 rounded text-xs font-mono overflow-x-auto break-words">
                        {selectedSurvey[key]}
                      </pre>
                    </div>
                  ))}
